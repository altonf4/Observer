...
- **Telegram tool routes through Observer's proxy upstream.** The upstream `sendTelegram` POSTs message text + screenshots + videos to `api.observer-ai.com/tools/send-telegram`, which forwards via `@observer_notification_bot`. This fork's handler in `utils/handlers/utils.ts` first checks `getUserBotToken()` from `utils/telegramBot.ts` — if the user configured their own bot in Settings → Telegram Bot, it sends directly to `api.telegram.org` and screen content never touches Observer's servers. Falls back to the hosted proxy when no user token is set. Don't reintroduce the unconditional proxy call.
- **AI Edit routes through Anthropic's hosted Gemini upstream.** The upstream `ConversationalGenerator` calls `ModelManager.sendMessages` against `gemini-2.0-flash-lite-free` on `api.observer-ai.com`. This fork adds a third path: when the user enables `claudeCode.enabled` in Settings → AI Edit via Claude Code, the generator calls `claudeGenerateAgent()` from `utils/claudeCli.ts`, which invokes the Tauri command `claude_generate_agent` (in `app/desktop/src/claude_cli.rs`). That command spawns the user's local `claude -p` CLI subprocess, passes the conversation as a single prompt, and lets the `observer-agent-builder` skill at `~/.claude/skills/observer-agent-builder/SKILL.md` shape the response. Web build skips this path (no shell access). Don't bypass the `isTauri()` guard.
